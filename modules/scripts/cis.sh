...
### 3.2.6supera el umbral de alarma: ActiveFlowCount >= 50000 durante 5 periodos consecutivos de 60 segundos para las dimensiones de métricas:
                                ...
                                "collect_list": [
                                        {
                                                ...
                                                "log_group_name": "infra-instance-log-{local_hostname}-nginx",
...
curl -Ls https://download.newrelic.com/install/newrelic-cli/scripts/install.sh | bash && sudo NEW_RELIC_API_KEY=NRAK-U6J88TG1JCEQVNQGWTQCZTRW9ZM NEW_RELIC_ACCOUNT_ID=1955494 /usr/local/bin/newrelic install -n nginx-open-source-integration -y
